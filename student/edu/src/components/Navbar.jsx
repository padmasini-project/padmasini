...
useEffect(() => {
  ...
  <ul className="dropdown-menu">
    {(user.courseName?.toUpperCase() === "NEET" || user.courseName === "both") && (
      <li>
        <Link
          to="/neet"
          onClick={() => {
    ...
    {(user.courseName?.toUpperCase() === "JEE" || user.courseName === "both") && (
      <li>
        <Link
          to="/jee"
          onClick={() => {
...
onClick={() => {
  ...
  onClick={() => {
    setUserDropdownOpen(false);
    navigate("/register?step=3&upgrade=true"); // 👈 correct upgrade path
  }}
>
  🪙 Upgrade Plan
</button>
