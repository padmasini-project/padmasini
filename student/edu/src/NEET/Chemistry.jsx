...
import { chemistryExplanations } from './ChemistryExplanation';
import ChemistryExplanation from "./ChemistryExplanation";
...
const Chemistry = () => {
    ...
    const [completedSubtopics, setCompletedSubtopics] = useState(() => {
        const savedData = localStorage.getItem(`completedSubtopics_${userId}_chemistry`);
        return savedData ? JSON.parse(savedData) : {};
    });

    // Reset data on first login for new users
    useEffect(() => {
        const isFirstLogin = localStorage.getItem(`isFirstLogin_${userId}_chemistry`);
        if (isFirstLogin === null || isFirstLogin === "true") {
            localStorage.removeItem(`completedSubtopics_${userId}_chemistry`);
            localStorage.setItem(`isFirstLogin_${userId}_chemistry`, "false");
 ...
 const handleSubtopicClick = (subtopic, index) => {
    ...
    const markSubtopicComplete = () => {
        ...
        setCompletedSubtopics((prev) => {
            ...
            localStorage.setItem(`completedSubtopics_${userId}_chemistry`, JSON.stringify(newCompletedSubtopics));
