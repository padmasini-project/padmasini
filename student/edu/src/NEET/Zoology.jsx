...
const Zoology = () => {
     ...
     const [completedSubtopics, setCompletedSubtopics] = useState(() => {
         const savedData = localStorage.getItem(`completedSubtopics_${userId}_zoology`);
         return savedData ? JSON.parse(savedData) : {};
     });
// Reset data on first login for new users
    useEffect(() => {
        const isFirstLogin = localStorage.getItem(`isFirstLogin_${userId}_zoology`);
        if (isFirstLogin === null || isFirstLogin === "true") {
            localStorage.removeItem(`completedSubtopics_${userId}_zoology`);
            localStorage.setItem(`isFirstLogin_${userId}_zoology`, "false");
 ...
 const isTopicCompleted = (topic) => {
  ...
  const isTopicUnlocked = (index) => {
    ...
    return isTopicCompleted(zoologyTopics[index - 1]);
  };
