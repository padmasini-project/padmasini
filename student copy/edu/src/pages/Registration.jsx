...
const RegistrationFlow = () => {
 ...
 // Add this missing state
  ...
  const handleStepOneSubmit = (e) => {
    ...
    localStorage.setItem(
      "registeredUser",
      JSON.stringify({ firstname, lastname, email, mobile, password, confirmPassword })
    );
    setStep(2);
  };
 ...
 const handleFinalSubmit = (e) => {
  ...
  // ✅ Different validation for upgrade
  if (!isUpgrade) {
  ...
  // inside handleFinalSubmit (before saving to sessionStorage)
if (
  (selectedCourse === "JEE" ||
    selectedCourse === "NEET" ||
    selectedCourse === "Both") &&
...
updatedUser.standard = normalizedStandard;
...
setStep(3); // ✅ only go to step 3, don’t call API yet
};
  // ✅ Complete payment and registration
  const completePayment = (method) => {
  setPaymentMethod(method);
  // ✅ Quick-pay methods succeed instantly
...
const handleFinalPayment = async () => {
  ...
  setTimeout(async () => {
    setIsPaying(false);
    setPaymentSuccess(true); // ✅ Mark payment success
    alert(`Payment successful using ${paymentMethod}`);

    // ✅ Save user plan duration
    ...
    // ✅ Registration API call only after payment success
                ...
                <form onSubmit={handleFinalSubmit}>
                  ...
                  {(selectedCourse === "JEE" ||
                    ...
                    >
                      ...
                      {["11th", "12th", "Both (11th + 12th)"].map((std) => (
                  ...
                  <div className="student-navigation-buttons">
                    <button
                      type="button"
                      onClick={() =>
                        isUpgrade ? navigate("/home") : setStep(1)
                      }
                    >
                      Previous
                    </button>
