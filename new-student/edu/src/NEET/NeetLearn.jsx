...
const NeetLearn = () => {
  ...
  // Load completion data from localStorage `neet-completed-*`
  useEffect(() => {
    const loadProgressFromLocalStorage = () => {
      ...
      topics.forEach((topic) => {
        traverseTopics(topic.units, topic.unitName);
      });
  ...
  // Set quiz/explanation completion for reload support
  ...
  const resetProgress = () => {
  const confirmReset = window.confirm("Are you sure you want to reset all your NEET progress? This action cannot be undone.");
  if (!confirmReset) return;

  localStorage.removeItem(`completedSubtopics_${userId}_neet`);

  // Also remove all neet-completed-* keys
