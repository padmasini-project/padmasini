...
const Home = () => {
  ...
  if (!user) {
    ...
    navigate("/login"); // or "/signin" if your route is named that
    return;
  }
  ...
  if (!registeredCourse) {
    ...
    navigate("/register"); // 🔁 redirect to signin
    return;
  }

  if (registeredCourse === requestedCourse) {
    navigate(`/${requestedCourse}`);
  } else {
    alert(`Access denied: You are registered for ${user.courseName?.toUpperCase() || "your course"} only.`);
    navigate("/register"); // 🔁 redirect after alert
  }
};
