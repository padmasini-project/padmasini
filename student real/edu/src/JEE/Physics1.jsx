...
const Physics1 = () => {
    ...
    const [completedSubtopics, setCompletedSubtopics] = useState(() => {
        const savedData = localStorage.getItem(`completedSubtopics_${userId}_physics1`);
        return savedData ? JSON.parse(savedData) : {};
    });

    // Reset data on first login for new users
    useEffect(() => {
        const isFirstLogin = localStorage.getItem(`isFirstLogin_${userId}_physics1`);
        if (isFirstLogin === null || isFirstLogin === "true") {
            // Clear user-specific progress data
            localStorage.removeItem(`completedSubtopics_${userId}_physics1`);
            localStorage.setItem(`isFirstLogin_${userId}_physics1`, "false");
